...
class MainScreenProvider with ChangeNotifier {
  ...
  bool isHomaPageAnimationInProgress = false;

  MainScreenData mainScreenData =
      MainScreenData(apiVersion: '1.0', data: [], banners: []);

  void setPlaylistOnHomePage(int index, bool animated, BuildContext context) {
    ...
    // if (isHomaPageAnimationInProgress == false) {
    //   context
    //       .read<PlayerProvider>()
    //       .carouselSliderControllerMainCell
    //       .animateToPage(index, duration: const Duration(milliseconds: 500))
    //       .then((onValue) {
    //     isHomaPageAnimationInProgress = false;
    //   });
    //   isHomaPageAnimationInProgress = true;
    // }
  }
  ...
  Future<void> initMainScreenData(bool isFromInit, {var playerProvider}) async {
    ...
    String apiKey = '/api/v2/homepage/$languageCode';
    ...
    if (response != null && response.statusCode == 200) {
      ...
      if (errorTest['error'] != null) {
        ...
        if (playerProvider != null) {
          playerProvider.checkMiniPlayerStatus();
  ...
  void switchToDataByPlaylistId(int playlistId, BuildContext context) {
    for (int i = 0; i < mainScreenData.data.length; i++) {
      if (mainScreenData.data[i].id == playlistId) {
        setPlaylistOnHomePage(i, true, context);
        break;
      }
    }
  }
