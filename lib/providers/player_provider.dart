...
class PlayerProvider with ChangeNotifier {
  ...
  String playNowPreviousSongName = '';
  ...
  void setNoInternetItems() async {
    ...
    print('YYYYYYYYYYYYYYYYYY ${indexOfPlayingItemBeforeInternetOff}');
  ...
  void checkMiniPlayerStatus() {
    if (_audioPlayer != null && _audioPlayer.currentIndex != null) {
  ...
  playPause(BuildContext context) async {
    if (_audioPlayer.playing) {
      _internetOffPlayingState = false;
      print("Pause heree");
      _audioPlayer.pause();
    } else {
  ...
  void _playNowTimer() {
    if (_audioPlayer.playing &&
        currentPlayNowUrl != '' &&
        itIsStream == true) {
      getPlayNowData(currentPlayNowUrl);
    }
  }
    ...
    if (items.isNotEmpty) {
      ...
      if (items.first is MainData) {
        ...
        for (MainData mD in items) {
          MediaItem item = MediaItem(
              id: mD.streamLink,
              displayTitle: mD.playNowEndpoint,
              genre: 'MainData',
              //artist: 'Vasia',
              artUri: Uri.parse(Singleton.instance.checkIsFoolUrl(mD.cardImage)),
              title: mD.name);
          mainRadioList.add(item);
        }
        mediaLibrary.baseItems[MediaLibrary.titleIdRadio] = mainRadioList;
        updateCarPlayRadioList(mainRadioList, 0);
      }
    }
    // });
  }
  ...
  void playAllTypeMedia(
    ...
    try {
      _internetOffPlayingState = true;
      indexOfPlayingItemBeforeInternetOff = indexToPlay;
      if (items.isNotEmpty) {
  ...
  void _playOrSeekForTypeMediaItem(
    ...
    try {
      ...
      if (Singleton.instance.audioHandler.queue.value.isNotEmpty) {
        currentStreamUrl = items[indexToPlay].id;
        currentPlayNowUrl =  items[indexToPlay].displayTitle ?? "";
      ...
      carPlayModule.updatePlayingStatusOnListItems(items[indexToPlay].title, isFromCarPlay: isFromInitialTap);
      isFromInitialTap = false;
    } catch (e) {
      print('Error playing media: $e');
      CarPlayService.showMessage('Error playing media: $e');
    }
  ...
  void _playOrSeek(
    ...
    if (indexInQueue != -1) {
      ...
      carPlayModule.updatePlayingStatusOnListItems(items[indexToPlay].name,
          manual: manual, isFromCarPlay: isFromInitialTap);
       checkMiniPlayerStatus();
    } else {
      print("Pause heree2");
      _audioPlayer.pause();
...
////////MainData/////////
      ...
      if (items.first is MainData) {
        ...
        typeOfElementIsDetected = true;
        List<MediaItem> mainRadioList = [];
        currentPlayNowUrl = items[indexToPlay].playNowEndpoint;
        for (MainData mD in items) {
          MediaItem item = MediaItem(
              id: mD.streamLink,
              displayTitle: mD.playNowEndpoint,
...
///////InterviewData///////
      ...
      print("Coming herere11");
      if (typeOfElementIsDetected) {
        try {
          carPlayModule.updatePlayingStatusOnListItems(items[indexToPlay].name, isFromCarPlay: isFromInitialTap);
          isFromInitialTap = false;
        } catch (e) {}
        await _audioPlayer.seek(Duration.zero, index: indexToPlay);
        _audioPlayer.play();
  ...
  Future<int> _isMediaCurrentlyInQueue(
    ...
    if (listOfMedia.first is MainData) {
      ...
      if (listOfMedia.first is MainData) {
        MainData first = listOfMedia.first;
        MainData last = listOfMedia.last;
        if (Singleton.instance.audioHandler.queue.value.length == listOfMedia.length
        && Singleton.instance.audioHandler.queue.value.first.id == first.streamLink
        && Singleton.instance.audioHandler.queue.value.last.id == last.streamLink
        ) {
          index = 1;
          MainData data = listOfMedia[indexToPlay] as MainData;
          currentPlayNowUrl = data.playNowEndpoint;
        }
    }
  ...
  void carPlayPlay({required ContentType type, required String name}) async {
    if (type == ContentType.mainRadio) {
      ...
      if (radioList.isNotEmpty) {
        ...
        try {
          if (carouselSliderControllerAppBar.ready) {
    ...
    if (type == ContentType.interview) {
      ...
      playAllTypeMedia(interviewList, indexToPlay, interviewList[indexToPlay].title, '');
    }
  }
    ...
    if (_isInternetAvailable == true) {
      ...
      if (response != null && response.statusCode == 200) {
        ...
        if (errorTest['error'] != null) {
          ...
          if (playNowPreviousSongName != playNow.songTitle) {
            playNowPreviousSongName = playNow.songTitle;
...
/////////Car Play///////////
  ...
  void updateCarPlayRadioList(List<MediaItem> listToUpdate, int indexToPlay) {
    if (Platform.isIOS) {
      carPlayModule.updateRadioList(listToUpdate, indexToPlay,
          onRadioItemTap: (songName) {
        carPlayPlay(type: ContentType.mainRadio, name: songName);
      });
    }
  }
  ...
  void _updateCarPlayAudioPodcastsList(List<AudioPodcast> listToUpdate)  {
    if (Platform.isIOS) {
      carPlayModule.updateAudioPodcastsList(listToUpdate,
        ...
        carPlayPlay(type: ContentType.podcast, name: name);
      });
    }
  }
  ...
  void _updateCarPlayInterviewList(
    ...
    if (Platform.isIOS) {
      carPlayModule.updateInterviewList(listToUpdate,
        ...
        carPlayPlay(type: ContentType.interview, name: name);
      });
    }
  }
}
