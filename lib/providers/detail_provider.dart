...
class DetailProvider with ChangeNotifier {
  ...
  void openMediaDetail(dynamic media, BuildContext context) {
    if (media != null) {
      ...
      if (media is Episode) {
        if (media.contentData.cards.first.type.contains('video')) {
          Navigator.of(context).push(MaterialPageRoute(
              builder: ((context) => VideoPodcastDetailScreen(episode: media)),
      ...
      if (media is MainData) {
        Provider.of<MainScreenProvider>(context, listen: false).switchToDataByPlaylistId(media.id, context);
