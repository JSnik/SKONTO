...
class AudioPlayerHandlerImpl extends BaseAudioHandler
  ...
  final _player = Singleton.instance.audioPlayer;
  ...
  MediaItem _previousPlayNowMediaItem = MediaItem(id: '', title: '');
  ...
  AudioPlayerHandlerImpl(PlayerProvider pProvider) {
    _playerProvider = pProvider;
    _init();
  }
  ...
  Future<void> _init() async {
    ...
    await updateQueue(
        _playerProvider.mediaLibrary.baseItems[MediaLibrary.titleIdRadio]!);
    ...
    // session.devicesStream.listen((_) {
    //   // Provider.of<MainScreenProvider>(context, listen: false).getMainScreenData(context, true).then((value) => null);
    //   // Provider.of<PodcastsProvider>(context, listen: false).getAllPodcasts(context);
    // });
  ...
  Future<List<MediaItem>> getChildren(String parentMediaId,
    ...
    switch (parentMediaId) {
      ...
      default:
        ...
        return _playerProvider.mediaLibrary.baseItems[parentMediaId]!;
    }
  }
  ...
  ValueStream<Map<String, dynamic>> subscribeToChildren(String parentMediaId) {
    switch (parentMediaId) {
      ...
      default:
        return Stream.value(
                _playerProvider.mediaLibrary.baseItems[parentMediaId])
  ...
  Future<void> updateMediaItem(MediaItem mediaItem) async {
    ...
    if (_previousPlayNowMediaItem.id == '' ||
        _previousPlayNowMediaItem.album != mediaItem.album) {
      _previousPlayNowMediaItem = mediaItem;
      needUpdate = true;
    }
  ...
  void updateCurrentMediaItem(MediaItem newMediaItem) {
  ...
  Future<void> skipToNext() async {
    if (mediaItem.value != null ||
        mediaItem.value!.genre != null ||
        mediaItem.value!.genre == 'MainData') {
      _playerProvider.carouselSliderControllerMainCell
          .nextPage(duration: Duration(milliseconds: 500));
    }
    await _player.seekToNext();
    _playerProvider.currentStreamUrl = (mediaItem.value?.id ?? "");
    _playerProvider.currentPlayNowUrl = (mediaItem.value?.displayTitle ?? "");
    _playerProvider.carPlayModule
        .updatePlayingStatusOnListItems(mediaItem.value!.title);
  }
  ...
  Future<void> skipToPrevious() async {
    if (mediaItem.value != null ||
        ...
        mediaItem.value!.genre == 'MainData') {
      _playerProvider.carouselSliderControllerMainCell
          .previousPage(duration: Duration(milliseconds: 500));
    }
  ...
  @override
  Future<void> seek(Duration position) => test(position);
  ...
  Future<void> playFromMediaId(String mediaId,
    ...
    for (var mediaList in _playerProvider.mediaLibrary.baseItems.values) {
      if (mediaList.isNotEmpty) {
        for (var i = 0; i < mediaList.length; i++) {
          var media = mediaList[i];
          if (media.id == mediaId) {
            await updateQueue(mediaList);
            await skipToQueueItem(i);
            if (mediaItem.value != null &&
                mediaItem.value!.genre != null &&
                mediaItem.value!.genre == 'MainData') {
              _playerProvider.carouselSliderControllerAppBar.animateToPage(i);
            }
            _player.play();
            break;
          }
