...
class _SearchScreenState extends State<SearchScreen> {
      ...
      appBar: AppBar(
        leading: BackButton(
          onPressed: () {
            ...
            provider.getSearchResponseState = ResponseState.stateFirsLoad;
            provider.search = Search.empty();
  ...
  List<Widget> _children() {
    ...
    if (_text == '') {
      return [Center(child: Text(Singleton.instance.translate('word_to_search'),
          style: AppTextStyles.main14regular),)];
    } else {
      switch (provider.getSearchResponseState) {
        case ResponseState.stateFirsLoad:
          return [
            // Center(child: Text(Singleton.instance.translate('word_to_search'),
            //   style: AppTextStyles.main14regular),)
          ];
  ...
  void _openItem(int mediaId, String type, SearchItem item) {
    if (type == 'news' || type == 'vacancy') {
      ...
      Navigator.of(context).pop();
      context.read<PlayerProvider>().switchToTabBarItem(0);
      Provider.of<MainScreenProvider>(context, listen: false).switchToDataByPlaylistId(mediaId, context);
    } else {
